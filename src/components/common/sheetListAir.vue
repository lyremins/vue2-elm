<template>
    ...
    <div class="sheet-list">
        ...
        <div class="sheet-header" @click="toggleSheet">
            <i class="icon iconfont icon-enter" ref="toggleicon"></i>   <span class="sheet-header-span">飞机型号：{{data_item.model}} <span></span></span>
            <i class="sheet-header-i icon iconfont icon-setup" @click.stop="showSheetMenu(data_item.user_name)"></i>   </div>

        <div @click="toDetail(data_item.airplane_id)" v-if="showSheets" class="sheet-content">

            <div class="sheet-content-image">
                <img :src="imgBaseUrl + item.image_path" width="50" height="50" style="padding-top: 20px;overflow: hidden">  
                </div>

            <div class="sheet-content-middle">

                <p style="">飞机型号：{{data_item.code}}</p>
                <p style="">出厂号码：{{data_item.code}}</p>
                <p style="">部队编号：{{data_item.army_id}}</p>
                <p style="">所属单位：{{data_item.unit}}</p>
                <p style="">制造厂：{{data_item.factory}}</p>
                <p style="">出厂日期：{{data_item.date}}</p>
